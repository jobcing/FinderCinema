...
import org.itner.domain.SearchtableVO;
...
public class CinemaServiceImpl implements CinemaService {
	...
	@Override
	public SearchtableVO search(TimetableVO vo, MovieSearchDTO dto) throws Exception {
		SearchtableVO result = new SearchtableVO();
		result.setCinemaTitle(vo.getCinemaTitle()); // 영화관 이름은 그대로 설정
		
		String searchMovieTitle = dto.getMovieTitle(); // 사용자가 검색한 영화 제목
		String[] objectMovieTitle = vo.getMovie(); // 현재 시간표에 저장된 영화 제목들
		int idx = -1; // 영화 인덱스를 알려줄 변수
		
		for(int i = 0; i < objectMovieTitle.length; i++){ // 일치하는 영화제목이 있는지 검색
			if(objectMovieTitle[i] == searchMovieTitle){
				result.setMovie(objectMovieTitle[i]); // String을 저장해야하는데...
				idx = i; // 해당 인덱스 기억
			}
		}
		
		if(idx == -1){ // 영화관에 사용자가 검색한 영화가 없다면....
			
		} else{ // 영화관에 사용자가 검색한 영화가 존재한다면....
			MovieTimeVO objectMovieTime = vo.getMovietimeVO()[idx];
			String[] objectTimes = objectMovieTime.getTime();  // 해당 영화에 대한 시간표를 가져온다.
